$(function () {
    ...
    let pageSize = 3;
    // 列出商品列表的URL
    let listUrl = '/myCommunityApp/frontend/listproductsbyshop';
    // 默认的页码
    let pageNum = 1;
    // 从地址栏里获取ShopId
    ...
    let searchDivUrl = '/myCommunityApp/frontend/listshopdetailpageinfo?shopId=' + shopId;
    ...
    // $('#exchangelist').attr('href', '/myCommunityApp/frontend/awardlist?shopId=' + shopId);
    // 获取本店铺信息以及商品类别信息列表
            ...
            if (data.success) {
                ...
                data.productList.map(function (item, index) {
                    html += '' + '<div class="card" data-product-id='
                        ...
                        + ' uupdated</p>' + '<span>click</span>' + '</div>'
                        + '</div>';
                });
                // 将卡片集合添加到目标HTML组件里
